...
private:
  bool run_verification(sycl::queue &queue, SrcT ref_value,
    ...
    shared_vector<SrcT> shared_ref_data(1, ref_value,
    ...
    for (size_t i = 0; i < result.size(); ++i) {
      ...
      if constexpr (std::is_same_v<SrcT, DstT>) {
        if (!are_bitwise_equal(ref_value, result[i])) {
          fail_test(i, result[i], ref_value, src_data_type, dst_data_type);
        }
  ...
  bool fail_test(size_t index, SrcT retrieved, SrcT expected,
    ...
    const auto description =
        BroadcastCtorTestDescription<DstT, NumElems, TestCaseT>(

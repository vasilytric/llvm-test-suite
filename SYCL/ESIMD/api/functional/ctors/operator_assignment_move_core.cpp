...
// assignment operator from source to destination simd instance. It is expected
// for destination simd instance to store a bitwise same data as the reference
// one.
...
int main(int, char **) {
  ...
  typedef std::integral_constant<bool, true> expected_call_move_ctor_value;

  const auto types = get_tested_types<tested_types::all>();
  const auto dims = get_all_dimensions();

  passed &=
      for_all_types_and_dims<ctors::run_test, move_assignment,
